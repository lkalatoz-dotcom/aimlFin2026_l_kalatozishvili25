...
Fixed version - corrected threshold to eliminate false positives
...
# ============================================================================
# METHOD 2: IP-Ratio Detection (FIXED THRESHOLD)
# ============================================================================
print("\n" + "="*80)
print("METHOD 2: IP-RATIO DETECTION (FIXED THRESHOLD)")
print("="*80)
...
unique_ips_per_sec = per_sec['unique']
requests_per_sec   = per_sec['total']

ratio = requests_per_sec / unique_ips_per_sec
ratio = ratio[ratio.notna() & (ratio > 0)]

print(f"  [DEBUG] ratio max: {ratio.max():.2f}  (DDoS interval should show ~104)")
...
print(f"  Threshold (old): {ratio.mean() + 1.5 * ratio.std():.2f} req/IP  ← caused false positives")
print(f"  Threshold (new): {threshold_ratio:.2f} req/IP")
...
if combined_anomalies:
    ...
    for i, (start, end) in enumerate(intervals, 1):
        ...
        interval_traffic = traffic_1sec[
            (traffic_1sec.index >= start) & (traffic_1sec.index <= end)
        ]
...
axes[0].axhline(threshold_traffic, color='green', linestyle=':', linewidth=2,
                label=f'Threshold ({threshold_traffic:.2f})')
axes[0].axhline(traffic_1sec.mean(), color='orange', linestyle='--', linewidth=1,
                label=f'Mean ({traffic_1sec.mean():.2f})')
...
# Plot 3: Requests/IP ratio with FIXED threshold
axes[2].plot(ratio.index, ratio.values, color='orange', alpha=0.7, linewidth=1,
             label='Requests per IP')
axes[2].axhline(threshold_ratio, color='green', linestyle=':', linewidth=2,
                label=f'Threshold — fixed ({threshold_ratio:.2f})')
axes[2].axhline(ratio.mean() + 1.5 * ratio.std(), color='gray', linestyle=':', linewidth=1,
                label=f'Old threshold ({ratio.mean() + 1.5 * ratio.std():.2f}) — caused false positives',
                alpha=0.6)
...
axes[2].set_title('Method 2: IP-Ratio Detection (Fixed Threshold — No False Positives)',
                  fontweight='bold', fontsize=12)
axes[2].legend(fontsize=9)
axes[2].grid(True, alpha=0.3)

# Plot 4: Combined final result
axes[3].plot(traffic_1sec.index, traffic_1sec.values, color='blue', alpha=0.5,
             linewidth=1, label='Traffic')
...
if intervals:
    for idx, (start, end) in enumerate(intervals):
        axes[3].axvspan(start, end, alpha=0.3, color='red',
                        label='DDoS Interval' if idx == 0 else '')
        mid = start + (end - start) / 2
        seg = traffic_1sec[(traffic_1sec.index >= start) & (traffic_1sec.index <= end)]
        peak_val = seg.max() if len(seg) > 0 else 0
